1. Java中和CPP类似：
    * 如果没有自定义构造函数，那么会默认生成一个无参构造函数；一旦类中定义了至少一个构造函数,编译器将不再提供默认构造函数
      ...
      * 如果父类有自定义的无参构造函数或者没有自定义构造函数，那么子类可以不用显示调用父类无参构造函数(super())，因为此时会自动调用父类无参构造函数
...
5. <mark>Java中同一个类中的静态方法和普通方法的调用关系：</mark>
    * 普通方法之间可以直接相互调用
    * 类的static方法是不能直接调用非static方法的，static方法属于类本身，不需要实例化对象就能运行；非static方法属于对象，必须有对象才能调用，如果要运行就必须先实例化对象，再调用非static方法。这和CPP一样的
    * 普通方法是可以直接调用static方法的，不需要`类名.静态方法`调用
    * static方法之间也可以直接调用
