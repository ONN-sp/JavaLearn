...
11. <mark>java中不是说引用类型都是类，在引用类型中：</mark>
    * 类：String、Integer、自定义类、集合、Object
    * 接口：List、Map、Runnable 等，本身不是类
    * 数组：int []、String []、自定义类数组，独立类型
    * 枚举：enum 定义的枚举类型
12. int[] arr = {1,2,3}; 是 int[] arr = new int[]{1,2,3}; 的"简化语法糖"—— 编译器帮你省略了 new int[] 这部分代码，但编译后的字节码、运行时的内存行为，和显式写 new 完全一致，因此数组对象必然在堆上
13. package ByteDance.Learn是声明当前类属于ByteDance.Learn这个包，不是导入包。便于其他类进行导入这个包下的类（当前这个 HelloWorld 类，属于 ByteDance 这个顶级包下的 Learn 子包（包名层级用 . 分隔））
14. Java中栈上的变量：栈内存只存储方法执行期间的临时数据，所有内容都会随方法执行结束自动释放
    ...
    * 方法的临时计算值：方法执行过程中临时的计算结果（返回值），会存在栈中
    ...
    ```
15. Java中堆上的变量：堆内存存储，只有GC能回收
    * <mark>所有对象的成员变量（无论基本/引用类型）：类的成员变量属于对象的一部分，随对象存储在堆上</mark>(cpp中对象创建在栈上，所有成员变量也都在栈； 对象创建在堆上，成员变量都在堆上)
    ```java
    class Student {
    ...
    * 所有数组对象（包括基本类型数组）：Java数组是对象，无论存储基本类型还是引用类型，数组本身必在堆上
    ...
    ```
    * 字符串常量池（串池）：JDK 1.7 后，字符串常量池从方法区移到堆上，String str = "Java" 的 "Java" 存在堆的常量池中
16. <mark>串池是java为了节省内存，给字符串专门搞的一个共享缓存池，不存在的字符串就会在串池创建新的，存在的重复字符串只存一份</mark>
    * 双引号直接赋值的方法字符串自动进串池
    * new String()一定在堆里创建新对象，不会进串池
    * 字符串常量拼接会进串池
    * 字符串变量拼接不会进串池
    * 需要注意串池中的字符串常量和new出来的字符串变量`==`比较的时候会返回false
    ```java
    "abc" == "abc"        → true（地址一样）
    new String("abc") == "abc" → false（地址不同）
    new String("abc") == new String("abc") → false（地址不同）
    ```
    `==`比较的是地址，此时串池和堆里new出来的字符串对象地址不同；可以用`equals()`来比较，`equals()`比较的是内容
    * `.intern`方法强制把字符串丢进串池，`String s = new String("abc").intern();`
    ```java
    String s1 = "abc"; 
    String s2 = "abc";
    ...
    // "abc"  ← s1、s2、s4 都指向这里
    // 堆内存：
    // new String("abc")  ← s3 指向这里（独立对象）
    ```
17. GC:JVM 自动帮你清理堆内存里没用的对象，释放空间，防止内存泄漏、OOM，不用手动free/delete，Java 使用可达性分析判断垃圾，把堆分新生代和老年代，短命对象用复制算法快速回收，长命对象用标记整理回收，自动清理内存避免 OOM，Full GC 会暂停程序
18. <mark>JVM是Java虚拟机，JVM是运行Java程序的虚拟机，JVM是运行Java字节码的虚拟机：你写的 Java 代码（.java 文件）→ 编译成「通用的中间语言」（字节码 .class 文件）；JVM 负责把这个「中间语言」翻译成当前操作系统（Windows/Mac/Linux）能看懂的「本地语言」，然后让系统执行</mark>
19. .java->.class->jvm负责解析字节码、分配内存、创建对象、调度线程执行等。也就是：<mark>你的Java程序=JVM加载字节码后跑起来的业务逻辑</mark>。HSDB就算JVM内置调试工具，直接透视JVM运行态
20. java可以看成就是多了一层JVM的升级版CPP
21. <mark>jhsdb hsdb中hsdb是调试jvm底层的神器，它可以查看jvm运行内存（堆内存、栈内存、对象实例、常量池、方法区全都能看）、查看jvm底层对象架构、死锁排查、查看class字节码、加载情况、分析OOM、内存泄露、看jvm底层指令、本地方法</mark>。Java 程序跑在 JVM 里，HSDB 是钻进 JVM 内部，看你的 Java 代码在虚拟机里变成了什么、怎么存、怎么跑
22. JVM核心作用 
    * 跨平台（Java最核心的优势）：C/C++ 程序：编译后直接生成操作系统专属的可执行文件（Windows 是 .exe，Linux 是 .out），换系统就不能用；Java 程序：编译后生成「字节码（.class）」（跨平台的通用格式），只要对应系统装了 JVM，就能运行这个字节码 
    * 内存管理
...
23. <mark>java程序的完整执行流程：</mark>
    ...
    * 运行
    * 结束
24. JVM启动、类加载、链接、初始化这几个步骤是JVM负责执行的
25. <mark>Java编译(.java文件)=>字节码文件（.class 文件）=>JVM运行字节码</mark>
26. 编译期：编译阶段；运行期：其余部分
27. JDK11引入了一个便捷特性：`java Main.java`可以直接运行`.java`文件，无需先`javac`编译，但这个不是跳过编译，而是JVM会在内存中临时编译生成`.class`字节码，运行完成后，临时生成的`.class`文件会被销毁。本质还是“编译->运行"，只是编译过程被JVM隐式执行了，没有最终在硬盘上保存`.class`文件
28. JVM的内存区域是在JVM一启动就创建好的，但是各个区域里存放的具体数据（类信息、对象、栈帧）：是在类加载、线程运行、方法调用时才陆续创建、放进去的
29. JVM内存模型：栈、堆、本地方法栈（Native 方法（底层 C/C++）的执行数据（栈帧、局部变量））、程序计数器（记录当前线程执行的字节码行号，Native 方法时为 undefined，记录线程执行位置，支持线程切换）、方法区（元空间）
    * 栈、本地方法栈、程序计数器是线程私有（每个线程都有自己的这些内存区域）的
    * 堆、方法区是线程共享（多个线程共享的这些内存区域）的
30. Java 中有些方法是用 C/C++ 实现的（比如 System.currentTimeMillis()、Object.hashCode()），这些方法被称为「Native 方法」（方法名带 native 关键字）
31. 方法区（JDK8之后是元空间实现的，之前是永久代实现）：
...
32. 类加载是 JVM 将类的元数据加载到方法区的过程，是 “准备阶段”，不创建对象； 类实例化是基于已加载的类，在堆中创建对象的过程； 类加载是实例化的前提，但类加载后可以不实例化（比如仅使用静态成员），一个类只加载一次，但可实例化多次
33. Java中的常量池
    * class文件常量池：.java 编译成 .class 后，文件内部自带的一张常量表。不存在内存中，只是文件数据。 编译时生成，存在 .class 文件里
    * 运行时常量池：方法区中 
    * 字符串常量池：堆里
34. 变量是存储数值的空间，而不是那个数值
35. `public static void main(String[] args)`解释：
    ...
    * args的每一个元素都是引用类型
36. <mark>`final`关键字：可以修饰变量、方法、类</mark>
...
38. <mark>Java和CPP对于类的默认实例化的区别：</mark>
...
39. <mark>java普通类（除了枚举类）不能用隐式创建来实例化对象，必须用new关键字来实例化对象，而CPP是可以直接隐式实例化的，此时它就是栈上的对象</mark>。java里只有枚举才能隐式实例化：
    ```java
    enum Gender{
    ...
    ```
40. <mark>java的类实例化必须有小括号，就算是调用无参构造函数也必须有；而cpp在调用无参构造的时候可以不写小括号。如：</mark>
    ```java
    class Student {}
    Student stu = new Student();
    ```
...
42. CPP的类本身没有访问修饰符
43. 四类访问修饰符(public > protected > 默认 (包权限) > private)
...
44. <mark>一个java文件，只能有一个public类（类名和文件名一致），其余类只能用默认修饰符</mark>
45. <mark>类的访问权限可以低于成员变量/成员方法，在语法上没问题，但是要使用这个变量或者方法，肯定要通过类的对象来访问，所以至少要先能访问到这个类才行，不管是不是static都一样，对于static也需要先访问到类才行。因此，实际开发中类的访问权限需要≥类的成员变量/方法的访问权限</mark>
46. 类修饰符只是表示这个类本身能被访问的权限，并不是表示该类内的成员/方法的权限
    ```java
...
47. 方法内的局部变量是不能用访问修饰符的，因为访问修饰符（public、protected、private）只对类成员变量/类/方法有效。局部变量仅在方法内有效，无访问权限的概念
48. `static`也不可以修饰局部变量，局部变量是方法级别的临时变量，属于线程私有，和类无关。`static`修饰的是类级别的成员方法/成员变量/内部类；`final`可以修饰局部变量
49. static 不能修饰外部类（顶级类），但可以修饰成员内部类，成员内部类是外部类的成员，称为静态内部类，static 可将其绑定到外部类本身（而非外部类的对象）
    ```java
    // 外部类（不能加static）
    public class Outer {
    ...
    ```
50. 修饰符 public 对于 Java 25 的 main 方法是冗余的。因为此时JVM也能正常识别并执行它，因此public修饰符不再是必需的
51. Java中不能在方法内再定义方法。方法内可以定义局部内部类，但是局部内部类仅在方法内可见，无访问权限的概念，static也不可以用。此时只可用final和abstract（定义抽象局部内部类（需子类实现））
52. Java没有结构体
53. Java的一条语句可以定义多个变量，也可以连续赋值
54. 计算机中，任意数据都是以二进制的形式化存储的
55. Java中的long类型数据必须以L结尾，可以是大写，也可以是小写；float类型必须以f或者F结尾
56. Java不支持在类外部定义变量/函数/代码块等。因为Java是一门纯面向对象的变成语言，设计理念是一切皆对象/类，所有的程序逻辑、数据都必须封装在类或接口中（这和CPP不同，CPP中是支持全局变量的）
57. Java规定：所有独立运行的Java程序，必须从public static void main(String[] args)开始运行
58. Java数组是对象，CPP数组本质是一段连续内存+指针常量。Java的定义是[]跟在类型后；CPP是[]跟在变量名后。需要注意：<mark>java中数组定义左边只写类型，不能写长度，左边至少声明一个引用类型变量，只是一个引用标签，本身不存数据，因此不能写长度</mark>
    ```java
    // 推荐写法
    int[] arr;
    int[] arr = new int[5];
    int[5] arr = new int[5];// 这是错的，java中左边声明数组类型时方括号不能写长度
    int[] arr = {1,2,3};// 这是简化语法糖，自动省略了new int[]
    int[3] arr = {1, 2, 3};// 这是错的，java中左边声明数组类型时方括号不能写长度
    // 也能写但不推荐
    int arr[];// cpp是这样的
    ```
59. <mark>java中数组定义左边只写类型，不能写长度，左边只是声明一个引用类型变量，只是一个引用标签，本身不存数据，因此不能写长度；而CPP不一样，`类型 变量名 [长度]`，因为这一句是直接在栈上造出完整数组实体，定义即分配内存，必须当场定好大小</mark>
60. Java的标识符命名规范
    * 由数字、字符、下划线、美元符组成
    * 不能以数字开头
    * 不能是关键字
    * 区分大小写
61. Java不需要像CPP那样手动写析构函数，因为Java有字段垃圾回收机制(GC)
62. 如果一个Java文件中有两个类（但是只能有一个public类），那么在文件视图显示时会把两个类都显示出来，注意这不是有两个java文件。编译时，Java 编译器会为每个类生成独立的 .class 文件，可以在输出目录（out/）中看到
63. 在Java中，同一个文件夹、同一个包下的类，不需要import，直接用就行
64. 企业开发中大多都是一个类对应一个java文件
65. 输出方式：
    * System.out
...
66. <mark>java和CPP中数组和`String`的打印：</mark>
...
67. <mark>`System.out.println()/print()/printf()`在打印类类型对象的时候，该打印方法代码内部本身就调用了类类型对象的`toString()`方法，如果子类对象没有重写`toString()`方法，那么打印的是对象的内存哈希码，此时用的是`Object`类的`toString()`方法。需要注意的是：对于基本数据类型和数组这些非类类型是不会调用`toString()`的</mark>
68. <mark>java中集合的打印：集合类都重写了`toString()`方法，会打印内部元素，因此可以直接输出集合名，如`ArrayList、LinkedList、HashSet、HashMap`等</mark>
69. 输入方式：
    * Scanner
    ...
    * 文件输入输出：
      * FileWriter
      * FileRReader
70. System.in是标准输入流，即键盘
71. BufferedReader：是 Java IO 流中用来高效读取字符流的类，主要用来从控制台、文件、网络流等地方读取文本，特点是带缓冲区、读得快、支持按行读取
...
72. Java和CPP一样，整数相除得到的还是整数，会自动向下取整
73. System.out.println()不支持直接用逗号分割多个字符串拼接参数，它只接收一个参数
    ```java
    ...
    ```
74. 数字运算：类型不一样不能运算，需要转成同类型的才能计算
75. Java的隐式转换是从小到大转换，类型的取值范围：byte<short<int<long<float<double。Java的隐式转换：
    * 自动进行转换的，会默认进行的
    ...
    * 把取值范围小的提升为取值范围大的，再进行运算
    ...
    int c = a + b;
    ```
76. Java的强制转换(有可能会丢失精度)：强制转换不会自动触发，需要手动书写代码。格式为：目标数据类型 变量名 = (目标数据类型)被强制转换的数据;
77. 大写字符->小写字符，用运算实现：char cc = (char)(c+32);// 必须要用强制类型转换，不然右边就是int类型
78. 字符串只有+操作，即拼接操作，没有其它操作。任意数据+字符串都是拼接操作，并产生一个新的字符串：10+8+"岁"->"18岁";10+8+"岁"+1+2->"18岁12"(后面1,2有左操作数是字符串所以说拼接)
79. 三元运算符（条件表达式）：条件 ? 运算1 : 运算2
80. <mark>Java中的boolean和int之间不能隐式转换也不能强制转换，这和CPP不同</mark>
    ```java
    // 下面这是错的
    int num;
    ...
    if(i) {...}
    ```
81. 为什么值传递形参的修改不对实参产生影响?
    形参是独立的新变量，和实参只是值一样，不是同一个内存地址，所以改形参，完全碰不到实参
82. <mark>Java只有值传递，和CPP不一样（CPP有引用传递）。Java中想在方法里修改、让外面也生效：</mark>
    * 传数组(也是引用类型)、自定义对象（引用类型）。java虽然只有值传递，但是传引用类型时，传的不是对象本身，而是对象地址的副本，此时是同一个地址
    ...
    ```
    * 方法return新值，外面接收赋值
83. 调用方法其实就是实参给形参赋值，基本类型是传值的副本，引用类型是传地址的副本
84. `@IntrinsicCandidate`是jdk16新加的，表示这个方法是JVM 内部内置方法，JVM 会特殊优化这个方法，不进方法栈，不按普通方法调用 ，直接一条 CPU 指令算出结果，非常快。类似cpp中gcc的内置函数
85. <mark>java10正式推出`var`关键字，用于自动推导变量类型，和cpp中`auto`一样</mark>
86. <mark>`instanceof`用于判断一个对象是不是某个类/接口的实例</mark>
87. <mark>在一个常见用maven管理的java项目中的项目结构包含：</mark>
    * `src/main/java/`：下面放核心业务代码
    * `src/test/java/`：下面放测试代码
    ![img.png](项目结构.png)
88. <mark>如果方法参数是： 一个接口/一个父类。那么你完全可以传入：它的实现类（子接口实现类）/它的子类，这就是 Java 的 向上转型（自动转型），也是多态的基础</mark>
89. <mark>IDEA快捷键，一个方法.var可以自动推导返回值类型，不需要手动写类型</mark>
90. <mark>`alt+(fn)+enter`可以将异常添加到方法签名中</mark>
91. <mark>`cltr+h`可以访问当前类/接口的子类型/实现类</mark>
92. JavaBean 就是一套符合特定编码规范的普通 Java 实体类，专门用来封装数据，充当数据载体，常用来存接口参数、数据库表数据、页面表单数据
93. `ctrl+O`：列出当前类所有父类、接口中可重写 / 实现的方法
94. `alt+enter`放在子类名上：打开当前类需要实现的方法有哪些，可以快速先写出来
95. `alt+(fn)+insert`:弹出Generate 代码生成菜单，包括构造器、`toString()`等，一键自动生成实体类必备模板代码，不用手写
96. `ctrl+p`可以显示当前方法中所需的参数形式