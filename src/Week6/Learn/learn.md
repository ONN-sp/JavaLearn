...
25. <mark>java中和CPP一样也自动向上类型转换:将派生类引用转换为基类的引用,这是自动进行的(<=>指向基类的引用可以引用派生类对象);java的向下类型转换(有风险)，必须手动强制转换（`子类 子类对象变量=(子类)父类类型的变量`）：</mark>
    ...
    Zi z = (Zi) f;
    ```
26. Java 普通方法默认可重写（因为java中普通方法就是虚方法）、默认动态绑定，不需 virtual；CPP 默认静态绑定，必须父类加 virtual 才能重写 + 运行时多态
27. 父类构造方法不会被继承
28. this和super用法汇总
...
49. <mark>多态调用成员的特点（java和Cpp都是）：</mark>
    ...
    * 不能调用子类独有的成员方法：`父类名称 对象名称=new 子类名称(); 父类对象名称.成员方法();`：此时访问不了子类独有的方法，需要向下强制类型转换
    ```java
    class Father {}
...
66. <mark>java的抽象方法是无方法体的方法，而cpp的虚方法是有方法体的</mark>
67. <mark>CPP中抽象类就是用于接口了，而java中为什么有了抽象类，还要有接口？
    C++ 单靠抽象类就能搞定一切，因为 C++ 支持多继承。Java 是单继承，抽象类只能继承一个，而接口中一个实现类是可以实现多个接口的</mark>
68. <mark>接口的特点：</mark>
    * 接口不能实例化，和抽象类一样
    * 接口的实现类，要么重写接口中所有的抽象方法，要么实现类是一个抽象类，还是不能实例化
    * 接口不是继承体系
    * 接口不能用`private、protected、final、static`。接口只用`public`/包访问权限（不写就是），不会自动补全`public`；JDK7以前接口方法默认强制是`public abstract`，可以不用写，默认自动补全，此时接口方法不能是`static、final、private、protected`
    * JDK8引入了默认方法，`default void run(){}`，有方法体，实现类可以重写也可以不重写，重写时在实现类中要去掉`default`，它默认强制是`public`的，可以省略`public`；也有静态方法，用接口名之间调用，实现类不重写，默认强制是`public`的，也可以省略`public`（不常用）
...
69. 接口的优点：
    ...
    * 高约束高安全性
    * 简化多态使用
70. <mark>接口也有多态的应用，`接口引用类型 对象名称 = new 实现类类型()`，如:</mark>
    ```java
    interface Flyable {
        void fly(); // 抽象方法
    }
    ...
    ```
71. <mark>抽象类常用于：有父子从属关系（学生是人的一种，明显的父子关系）等、有共同对象属性（学生和老师都有name、age）、有共用代码；接口常用于：没父子从属关系、纯行为、没有关系的类可以共用、要多套额外能力（可单实现、也可多实现）</mark>。需要注意不一定要每个点都满足才能用抽象类或者接口。举个例：
    ```java
    // 抽象父类：动物
    ...
    ```
72. 为什么有内部类？把一个类，藏在另一个类里面，只为了方便、专属、省事。比如汽车的发动机、人的心脏等：
    ```java
    public class Car {
        String carName;
        int carAge;
    ...
    ```
73. <mark>内部类表示的事务是外部类的一部分，内部类单独出现没有任何意义</mark>
74. 内部类的分类
    ...
    * 匿名内部类:一个没有名字的内部类。在继承一个类或者实现一个接口的时候少写一个java文件，这也是Lambda表达式的前提。定义格式：
      ...
      * 匿名内部类如果是函数式接口的实现类，此时就可以用Lambda表达式简化，如`Swim s = new Swim() {...};`
