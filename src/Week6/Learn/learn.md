...
16. <mark>重写必须先继承类或实现接口，这样才能重写/实现方法</mark>
17. 重写方法的名称、形参列表必须与父类中的一致，方法体按照实际需要书写
18. `@Override`这是一个注解，是给JVM看的，表示下面的方法是重写的父类的方法。CPP方法重写是父类(用virtual)、子类(用override)
19. 如果父类里面的代码，一行都不用，此时把子类中的方法体重新完整写一遍即可；如果父类里面的代码还想用，此时只是在父类继承上再加其它逻辑。那么就可以先通过super关键字调用父类的方法得到一个结果，再堆这个结果进行操作
20. final修饰类为最终类，里面所有方法不能被重写，这个类也不能被继承
21. <mark>private方法、static方法、final方法都不能被重写</mark>
    ...
    * final方法本身就规定不能再修改
22. 重写和重载是不一样的概念
23. 多态允许相同的操作在不同的对象上表现出不同的行为,分为:静态多态(编译时多态)和动态多态(运行时多态)
24. <mark>java中静态多态用方法重载来实现；运行时多态用继承+重写来实现:</mark>
    ```java
    // 编译时多态
    ...
    ```
25. <mark>开发里99% 场景重写都配合多态一起用（类似CPP的虚函数/重写配合多态一样）</mark>
26. <mark>java中和CPP一样也自动向上类型转换:将派生类引用转换为基类的引用,这是自动进行的(<=>指向基类的引用可以引用派生类对象);java的向下类型转换(有风险)，必须手动强制转换（`子类 子类对象变量=(子类)父类类型的变量`）：</mark>
    ```java
    // 子类类型 变量名 = (子类类型) 父类引用;
    class Fu {}
    class Zi extends Fu {}
    ...
    Zi z = (Zi) f;
    ```
27. <mark>Java 普通方法默认可重写（因为java中普通方法就是虚方法）、默认动态绑定，不需 virtual；CPP 默认静态绑定，必须父类加 virtual 才能重写 + 运行时多态</mark>
28. 父类构造方法不会被继承
29. this和super用法汇总
    ![img.png](this和super.png)
30. 如果在构造方法中写上了this(),就不能再写super()了，JVM也不会自动添加super()
31. 子类继承≠子类调用
32. <mark>java的虚方法就是普通成员方法，非final、static、private</mark>
33. static成员变量继承也只会在父类和子类中共享
34. 子类真正能继承父类的：
...
35. 虚方法的继承规则就是利用虚方法表，而方法重写就是替换虚方法表中对应虚方法中的地址
    ![img.png](虚方法继承规则.png)
36. java的虚方法表和CPP的虚函数表类似
    * 都是为了实现运行时多态
    * 都是在类加载阶段创建
    * 每个类一张表
    * 表内存放方法的地址（入口指针）
...
37. cpp中不加 virtual 的方法 不进表，没有多态；java中private、static、final方法不进表
38. <mark>如果子类没有新增任何虚方法、也没有重写任何方法： Java 和 Cpp 都不会新建表，直接和父类共享同一张虚方法表</mark>
39. <mark>java和cpp都是：一个类的虚表只会有一份,就算这个类实例了很多对象,它们也是共享一个虚表的</mark>
40. 只有虚方法表里面的方法才能够被重写，方法重写的实际意义就是替换需方发表中记录方法的内存地址（CPP也只有虚函数才能被重写）
41. <mark>java中每个new出来的对象在对象头都有一个Klass Pointer，用于指向存在元空间的虚表</mark>
42. <mark>java语法层面是没有指针的，而底层JVM虚拟机层面是有指针的，因为是C++代码</mark>
43. <mark>对象头的Klass指针存在每个对象中，在堆里；而CPP的虚表指针是类的非静态成员变量，new出来的对象就在堆，局部变量对象在栈里</mark>
44. <mark>java的虚表在元空间，类元数据里，元空间属于JVM的本地内存，不是堆、不是栈、不是方法区，是操作系统直接给JVM的堆外内存，所以虚表在堆外内存里；CPP虚表在只读数据段</mark>
45. <mark>java的多态原理和cpp基本一样，共同点</mark>
     * 每个类一张表，同类所有对象共享同一张表，对象只存指针指向表
     * 父类引用指向子类对象 → 运行时顺着对象找子类的表 → 执行子类重写的方法
     * 子类先拷贝父类表，重写就覆盖表项，不重写就复用父类地址； 子类新增方法才会扩建表
46. <mark>java的多态和cpp不同点：</mark>
...
47. <mark>java和CPP中如果没有虚表和虚表指针也是可以访问到父类方法的，只是要一级一级往上查找，但是有了虚表指针和虚表就可以直接访问自己的表，加快了速度，空间换了时间</mark>
48. <mark>类继承多态的表现形式：`父类名称 对象名称=new 子类名称();`</mark>
49. 多态的前提：
    * 有继承/实现关系
    * 有父类引用指向子类对象（向上类型转换）
    * 有方法重写（不是必须的）
50. <mark>多态调用成员的特点（java和Cpp都是）：</mark>
    ...
    * （多态弊端）不能调用子类独有的成员方法：`父类名称 对象名称=new 子类名称(); 父类对象名称.成员方法();`：此时访问不了子类独有的方法，需要向下强制类型转换
    ...
    ```
51. <mark>向下类型转换时，一般会先判断运行类型是否正确。java判断判断左边对象是不是右边类/接口的实例的类型`instanceof`</mark>
    ```java
...
52. <mark>抽象方法：将共性的方法抽取到父类后。因为每一个子类的方法体是不一样，所以在父类中不能确定具体的方法体，只有方法名没有方法体。该方法就可以定义为抽象方法（类似cpp的纯虚函数）</mark>
    ...
    ```
53. <mark>抽象类：如果一个类中存在抽象方法，那么该类就必须声明为抽象类（类似cpp的抽象类）</mark>
    ```java
    public abstract class 抽象类名 {}
    ```
54. <mark>final、private、static不能和抽象方法/类结合使用</mark>。因为抽象方法/类需要被重写</mark>
55. <mark>抽象类可以继承自普通类，抽象类中可以没有抽象方法</mark>
56. <mark>抽象类被继承后如果是普通类(没加`abstract`关键字)，那么必须把父类的抽象类都要实现；如果子类也是`abstract`类，那么子类可以不用全部实现/或者不实现父类的抽象方法。CPP也是的，只是说CPP没有`abstract`关键字，而是在方法内用`virtual`关键字来声明抽象方法</mark>
57. <mark>java中抽象类特点：</mark>
...
58. 抽象类和抽象方法的优点：
    ...
    * 抽象类兼具 模板 + 规范 双重能力
    * 抽象类降低耦合，便于扩展
59. <mark>接口就是一个规则，而且是独立于继承体系以外的规则</mark>
60. <mark>接口用关键字interface来定义`public interface 接口名{}`</mark>
61. <mark>接口和类之间是实现关系，通过implements关键字表示`public class 类名 implements 接口名{}`</mark>
62. 接口和实现一起的类，`public class 类名 extends 抽象类 implements 接口名{}`
63. <mark>接口的抽象方法（JDK8还有默认方法）是通过重写来完成实现的</mark>
64. 接口也是一种引用类型
65. <mark>重写方法的访问权限必须≥抽象方法，因此接口的实现类中的重写方法访问权限只能是`public`</mark>
66. 重写的方法要把`abstract`去掉，返回值类型要一模一样，参数列表完全一致
67. <mark>java中虚方法和抽象方法都是可以被重写的，而抽象方法必须重写，虚方法可以被重写也可以不用</mark>
68. <mark>java的虚方法<=>cpp的虚方法；java的抽象方法<=>cpp的纯虚方法</mark>
69. <mark>CPP中抽象类就是用于接口了，而java中为什么有了抽象类，还要有接口？
...
70. <mark>接口的特点：</mark>
    ...
    * JDK8引入了默认方法，`default void run(){}`，有方法体，因此就不是抽象方法了，实现类可以重写也可以不重写，重写时在实现类中要去掉`default`，它默认强制是`public`的，可以省略`public`；也有静态方法，用接口名直接调用，实现类不重写，默认强制是`public`的，也可以省略`public`（不常用）
    * 对于`default`默认方法，如果多个接口中存在相同名字的默认方法，子类就必须对该方法进行重写
    * 为了在接口中抽取重复代码，JDK9在JDK8基础上又引入`private`私有方法，仅供接口内部调用，实现类访问不到（不常用）
    * 接口内的成员变量默认强制是`public static final`，只能是常量，可以不用写，默认自动补全。外部获取可以直接`接口名.成员变量名`
    * 接口没有构造方法
    ...
    * 方法是接口引用类型，那么传入的要是实现了该接口的实例（不是该接口类型本身的实例），包含独立实现类、匿名内部类、Lambda（函数式接口）、子接口实现类
71. 接口的优点：
    * 突破单继承限制
    ...
    * 高约束高安全性
    * 简化多态使用
72. <mark>接口也有多态的应用，`接口引用类型 对象名称 = new 实现类类型()`，如:</mark>
    ```java
    interface Flyable {
        void fly(); // 抽象方法
    }
    ...
    ```
73. <mark>抽象类常用于：有父子从属关系（学生是人的一种，明显的父子关系）等、有共同对象属性（学生和老师都有name、age）、有共用代码；接口常用于：没父子从属关系、纯行为、没有关系的类可以共用、要多套额外能力（可单实现、也可多实现）</mark>。需要注意不一定要每个点都满足才能用抽象类或者接口。举个例：
    ```java
    // 抽象父类：动物
    ...
    ```
74. 为什么有内部类？把一个类，藏在另一个类里面，只为了方便、专属、省事。比如汽车的发动机、人的心脏等：
    ```java
    public class Car {
        String carName;
        int carAge;
    ...
    ```
75. <mark>内部类表示的事务是外部类的一部分，内部类单独出现没有任何意义</mark>
76. 内部类的分类
    ...
    * 局部内部类：写在方法内部，属于局部变量，和局部变量同等级。外界是无法直接使用，使用的话只能在写局部内部类的方法的内部创建对象并在方法内使用。该类可以直接访问外部类的成员，也可以访问方法内的局部变量
      ...
      ```
    * 匿名内部类:一个没有名字的内部类。在继承一个类或者实现一个接口的时候少写一个java文件，这也是Lambda表达式的前提，专门用来临时实现接口/继承抽象类/继承普通类，不用单独写一个.java 类文件。如果工作中需要多次使用接口的实现类或者类的继承类对象，那么最好新建一个java文件，进行后续调用。定义格式：
      ```java
      new 类名或者接口名() {
        重写方法;
      }
